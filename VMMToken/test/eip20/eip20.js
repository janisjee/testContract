...
const VMMAbstraction = artifacts.require('VMM');
let HST;

contract('VMM', (accounts) => {
  beforeEach(async () => {
    HST = await VMMAbstraction.new(10000, 'Simon Bucks', 1, 'SBX', { from: accounts[0] });
  });
  ...
  it('creation: should succeed in creating over 2^256 - 1 (max) tokens', async () => {
    // 2^256 - 1
    const HST2 = await VMMAbstraction.new('115792089237316195423570985008687907853269984665640564039457584007913129639935', 'Simon Bucks', 1, 'SBX', { from: accounts[0] });
